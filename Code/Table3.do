//do "/Users/shoude/Dropbox/eegap/Replication_JPubE_RR/Code/Table3.do"
...
eststo: ppmlhdfe sales_hd real_price_zip_mode eleccost_county rebate_estar s_estar, absorb(i.county_utility pid_id week_num) vce(cluster county_utility)
...
// countyXyear, week, pid
eststo: ppmlhdfe sales_hd real_price_zip_mode eleccost_county rebate_estar s_estar, absorb(i.county_utility##i.year pid_id week_num) vce(cluster county_utility)
estadd local fixed "", replace
estadd local fixed0 "Yes", replace
estadd local fixed1 "Yes", replace
estadd local fixed2 "Yes", replace
...
eststo: ppmlhdfe sales_hd real_price_zip_mode eleccost_county rebate_estar s_estar, absorb(i.county_utility##i.year pid_id brand_week_num) vce(cluster county_utility)
estadd local fixed "", replace
estadd local fixed0 "Yes", replace
estadd local fixed1 "Yes", replace
estadd local fixed2 "Yes", replace
...
eststo: ppmlhdfe sales_hd real_price_zip_mode eleccost_county rebate_estar , absorb(i.county_utility##i.year i.county_utility##i.s_estar i.county_utility##i.size_id i.county_utility##i.type_id i.county_utility##i.brand_id_num pid_id brand_week_num) vce(cluster county_utility)
estadd local fixed "", replace
estadd local fixed0 "Yes", replace
estadd local fixed1 "Yes", replace
estadd local fixed2 "Yes", replace
...
cd $pathresults
 esttab using "Table_paper_main_poisson_sample50.tex", replace legend label star(* 0.1 ** 0.05 *** 0.01) nonumbers mtitles("(1)" "(2)" "(3)" "(4)") se keep(real_price_zip_mode eleccost_county) /// 
...
"\begin{tabular*}{1.0\hsize}{@{\hskip\tabcolsep\extracolsep\fill}lcccc}" "\toprule \\") ///
 ...
 "\par \noindent \footnotesize {Notes: The models are estimated using a Poisson regression.  The dependent variable is the number of units of a particular appliance sold in a given week in a given zip code.  The standard errors are clustered at the county level and are in parentheses.  ***, ** and * denote statistical significance at the 1, 5 and 10 percent levels.  The valuation ratios are computed assuming a discount rate of 5\% and a refrigerator lifetime of 12 years.}" "\end{table}") ///
  title(Estimation of the Effect of Price and Energy Costs on Demand) s(fixed fixed0 fixed1 fixed2 fixed3 fixed4 fixed5 N, label("\underline{Fixed Effects}" "Product" "County" "Week" "County $\times$ Year" "Brand $\times$ Week" "County $\times$ Efficiency Attributes" "Observations"))
